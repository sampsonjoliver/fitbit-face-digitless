...
export function getDateInWordsInstance(
  is12h: boolean,
  date: Date,
  locale: string
) {
...
export class DateTimeInWords12h {
  ...
  protected formatNumber(n: number, onesPrefix?: string): string {
    let units = n % 10;
    ...
    switch (tens) {
      case 0:
        return `${onesPrefix ? onesPrefix + ' ' : ''}${
          onesTable[this.language][units]
        }`;
      case 1:
    ...
    if (this.language === 'it') {
      ...
      if (units === 1 || units === 8) {
        return `${tensTable[this.language][tens].slice(0, -1)}${
          onesTable[this.language][units]
        }`;
      }
  ...
  formatMinutes(): string {
    ...
    return this.formatNumber(this.date.getMinutes(), 'zero');
  }
  ...
  formatDate(): string {
    return `${zeroPad(this.date.getDate())}.${zeroPad(
...
export class DateTimeInWords24h extends DateTimeInWords12h {
  ...
  formatHours(): string {
    return (
      this.formatNumber(this.date.getHours()) || midnightTable[this.language]
    );
  }
